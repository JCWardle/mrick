-- ============================================================================
-- FUNCTION: get_matched_cards
-- Returns a list of card titles that both partners have matched on (both 'yum')
-- Respects RLS by only allowing users to see matches with their own partner
-- ============================================================================

CREATE OR REPLACE FUNCTION get_matched_cards(user_uuid UUID DEFAULT auth.uid())
RETURNS TABLE (
  card_id UUID,
  card_title TEXT
)
...
COMMENT ON FUNCTION get_matched_cards(UUID) IS 
  'Returns card titles that both partners have matched on (both responded "yum"). Only allows users to see matches with their own partner. Uses SECURITY INVOKER to respect RLS policies - users can only see their own swipes and their partner''s swipes through existing RLS policies.';
