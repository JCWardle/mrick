-- ============================================================================
-- FIX AMBIGUOUS COLUMN REFERENCE IN accept_invitation_by_code FUNCTION
-- Fixes the "column reference 'id' is ambiguous" error
-- ============================================================================

-- Update the function to fix the ambiguous column reference
...
BEGIN
  ...
  WHERE UPPER(partner_invitations.code) = UPPER(invitation_code)
    AND partner_invitations.used_at IS NULL
    AND partner_invitations.expires_at > NOW()
  LIMIT 1;
  ...
  -- Update the invitation atomically (use table alias to avoid ambiguity)
